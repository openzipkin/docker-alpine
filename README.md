...
```bash
docker run -ti --rm ghcr.io/openzipkin/alpine:3.23.3
/ #
```
...
```bash
# Note 3.23.3 not 3.23!
./build-bin/build 3.23.3
```
...
To release the image, push a tag matching the arg to `build-bin/build` (ex `3.23.3`).
